<div>
    <template id="reportePresupuesto">
        <swal-html>
            <div class='flex items-center justify-center mt-4'>
                <div
                    ...
                    <section class="h-full flex flex-col items-start justify-end gap-1">
                        <h1 class="text-xl font-semibold text-zinc-800 antialiased">Reportaron el siguiente Presupuesto</h1>
                        <p class="text-lg font-medium text-lime-600 antialiased">${{$presupuesto}}</p>
...
@push('scripts')
    ...
    <script>
        ...
        function mostrarMotivo(motivo){
            Swal.fire({
                template: '#reportePresupuesto'
            }).then((result) => {
                if(result.isConfirmed) {
                    Livewire.emit('autorizarPresupuestoCliente')
                    Swal.fire(
                    'Presupuesto Autorizado',
                    'Una vez autorizado el presupuesto el centro de servicio puede continuar con la orden.',
                    'success'
                    )
                    setTimeout(function(){
